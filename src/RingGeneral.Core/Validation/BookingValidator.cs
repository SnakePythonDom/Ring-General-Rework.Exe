...
public sealed class BookingValidator
{
    public ValidationResult ValiderBooking(BookingPlan plan)
    {
        ...
        {
            var workerAppearances = new Dictionary<string, int>();
            var workerMatchCount = new Dictionary<string, int>();
            ...
            foreach (var segment in plan.Segments)
            {
                ...
                {
                    // Track appearances
                    workerAppearances[participantId] = workerAppearances.GetValueOrDefault(participantId) + 1;
                    if (segment.TypeSegment?.ToLower() == "match")
                    {
                        workerMatchCount[participantId] = workerMatchCount.GetValueOrDefault(participantId) + 1;
                    }
                    ...
                    if (health.Blessure is not "AUCUNE" && health.Blessure is not null)
                    {
                        issues.Add(new ValidationIssue(
                            ValidationSeverity.Erreur, // Promoted to Error if booking injured
                    ...
                    if (health.Fatigue >= 70)
                    {
                        issues.Add(new ValidationIssue(
                            ...
                            $"Risque : {participantId} est très fatigué ({health.Fatigue}%). Performance réduite.",
                            segment.SegmentId));
                    }
                    else if (health.Fatigue >= 50 && segment.Intensite >= 70)
                    {
                        issues.Add(new ValidationIssue(
                            ValidationSeverity.Avertissement,
                            "segment.participant.fatigue.risk",
                            $"Risque : {participantId} ({health.Fatigue}% fatigue) booké dans un match intense.",
                            segment.SegmentId));
                    }
                }
            }
            ...
            // Check for over-use
            foreach (var (workerId, count) in workerAppearances)
            {
                if (count > 3)
                {
                    issues.Add(new ValidationIssue(
                        ValidationSeverity.Avertissement,
                        "booking.worker.overuse",
                        $"Exposure : {workerId} apparaît dans {count} segments. Risque de saturation du public."));
                }
            }
            foreach (var (workerId, count) in workerMatchCount)
            {
                if (count > 1)
                {
                    issues.Add(new ValidationIssue(
                        ValidationSeverity.Avertissement,
                        "booking.worker.matches",
                        $"Fatigue : {workerId} participe à {count} matchs ce soir. Risque élevé de blessure."));
                }
            }
        }
