...
public interface IRelationsRepository
{
    ...
    /// </summary>
    List<WorkerRelation> GetRelationsForWorker(string workerId);

    /// <summary>
    /// Get relations of a specific type for a worker
    /// </summary>
    List<WorkerRelation> GetRelationsByType(string workerId, RelationType relationType);

    /// <summary>
    /// Add or update a relation between two workers
    /// </summary>
    ...
    /// </summary>
    bool RelationExists(string workerId1, string workerId2);
    ...
    /// <summary>
    /// Récupère les cliques (factions naturelles) pour une compagnie.
    /// </summary>
    List<List<string>> GetCliques(string companyId, List<string>? relevantWorkerIds = null);
    void RefreshCliques(string companyId, List<string>? relevantWorkerIds = null);
    ...
    /// </summary>
    List<FactionMember> GetWorkerFactionHistory(string workerId);

    /// <summary>
    /// Get current faction memberships for a worker
    /// </summary>
    List<FactionMember> GetCurrentFactionMemberships(string workerId);
    ...
    /// </summary>
    bool IsWorkerInFaction(string workerId, int factionId);

    /// <summary>
    /// Get factions where worker is the leader
    /// </summary>
    List<Faction> GetFactionsLedByWorker(string workerId);
}
