...
using RingGeneral.UI.ViewModels.Common;
...
public sealed class WorkerDetailViewModel : ViewModelBase, INavigableViewModel
{
    ...
    private Common.WorkerProfileViewModel? _profile;
    ...
    public Common.WorkerProfileViewModel? Profile
    {
        get => _profile;
    ...
    private void LoadWorkerDetails(string workerId)
    {
        ...
        try
        {
            ...
            if (int.TryParse(workerId, out int idInt))
            {
                ...
                if (fullWorker != null)
                {
                    var names = _repository.ChargerNomsWorkers();
                    Profile = new Common.WorkerProfileViewModel(fullWorker, _repository, names);
                    profileLoaded = true;
                }
            }
            ...
            if (!profileLoaded)
            {
                ...
                // Use repository if available, or empty dict if not
                var names = _repository != null ? _repository.ChargerNomsWorkers() : new System.Collections.Generic.Dictionary<string, string>();
                Profile = new Common.WorkerProfileViewModel(fallbackWorker, _repository!, names);
            }
    ...
    private void LoadPlaceholderData()
    {
        ...
        // Pass repository if available, even if placebo, or null/mock?
        // WorkerProfileViewModel expects IGameRepository. _repository might be null.
        // If _repository is null, we can't really save.
        // We should try to use _repository if not null, otherwise... we might need a mock or handle null in VM (which implies VM changes).
        // But LoadPlaceholderData is usually called when something went wrong or offline.
        // Let's assume _repository is not null for now or pass null and fix VM if it crashes (it uses it in logging).
        // Actually VM constructor does `_repository = repository;`
        // Save command uses it.
        // We can pass _repository (nullable in DetailVM, but VM expects non-null Interface).
        // Let's assume for placeholder we might not have a functional repo.
        // But for compilation, we need to pass something.
        // If _repository is null, we are in trouble.
        // Let's pass _repository! (bang operator) and hope it's initialized or won't be used for Save during placeholder.
        Profile = new Common.WorkerProfileViewModel(dummyWorker, _repository!, names);
    }
