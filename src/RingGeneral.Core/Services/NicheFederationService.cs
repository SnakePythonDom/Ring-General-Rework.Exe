...
public class NicheFederationService
{
    ...
    public async Task<NicheFederationProfile> EstablishNicheAsync(string companyId, NicheType nicheType)
    {
        ...
        var profile = new NicheFederationProfile
        {
            ...
            NicheType = nicheType,
            CaptiveAudiencePercentage = 70 + new Random().Next(20), // 70-90%
            TvDependencyReduction = 40 + new Random().Next(30), // 40-70%
            MerchandiseMultiplier = 1.3 + (new Random().NextDouble() * 0.4), // 1.3-1.7
            TicketSalesStability = 60 + new Random().Next(30), // 60-90%
            TalentSalaryReduction = 20 + new Random().Next(20), // 20-40%
            TalentLoyaltyBonus = 30 + new Random().Next(30), // 30-60%
