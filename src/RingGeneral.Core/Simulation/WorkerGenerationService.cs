...
public sealed class WorkerGenerationService : IWorkerGenerationService
{
    ...
    public WorkerGenerationReport GenerateWeekly(GameState state, int seed)
    {
        ...
        if (generationYouthActive)
        {
            foreach (var youth in state.YouthStructures)
            {
                ...
                var paysUtilises = traineesParPays.TryGetValue(youth.Region ?? "Global", out var comptePays) ? comptePays : 0;
                var capPaysRestant = Math.Max(0, _spec.Caps.Trainees.ParPaysAnnuel - paysUtilises);
                if (capPaysRestant == 0)
                {
                    resultatStructures.Add(new WorkerGenerationResultatStructure(youth.YouthId, youth.Nom, youth.Region ?? "Global", 0, "Cap pays atteint."));
                    continue;
                }
                ...
                {
                    resultatStructures.Add(new WorkerGenerationResultatStructure(youth.YouthId, youth.Nom, youth.Region ?? "Global", 0, "Cap compagnie atteint."));
                    continue;
                }
                ...
                {
                    resultatStructures.Add(new WorkerGenerationResultatStructure(youth.YouthId, youth.Nom, youth.Region ?? "Global", 0, "Limites atteintes."));
                    continue;
                }
                ...
                if (quantite == 0)
                {
                    resultatStructures.Add(new WorkerGenerationResultatStructure(youth.YouthId, youth.Nom, youth.Region ?? "Global", 0, "Aucune génération cette période."));
                    continue;
                }
                ...
                if (youth.Region != null) traineesParPays[youth.Region] = (traineesParPays.TryGetValue(youth.Region, out var exist) ? exist : 0) + quantite;
                ...
                var message = $"{quantite} nouveaux élèves rejoignent {youth.Nom} ({youth.Region ?? "Global"}).";
                notices.Add(new WorkerGenerationNotice("youth", "Nouveaux trainees inscrits", message, $"youth:{youth.YouthId}"));
                resultatStructures.Add(new WorkerGenerationResultatStructure(youth.YouthId, youth.Nom, youth.Region ?? "Global", quantite, null));
            }
        }
    ...
    public WorkerGenerationReport GenerateInitial(YouthStructureState youth, int count, int semaine, int seed)
    {
        _random.Reseed(seed);
        var workers = new List<GeneratedWorker>();
        for (var i = 0; i < count; i++)
        {
            var worker = GenererWorker(youth, semaine, i, true);
            workers.Add(worker);
        }

        var results = new List<WorkerGenerationResultatStructure>
        {
            new(youth.YouthId, youth.Nom, youth.Region ?? "Global", count, "Génération initiale effectuée.")
        };

        return new WorkerGenerationReport(semaine, workers, results, new List<WorkerGenerationResultatMonde>(), new List<WorkerGenerationNotice>());
    }
    ...
    private GeneratedWorker GenererWorker(YouthStructureState youth, int semaine, int index, bool estTrainee)
    {
        ...
        var gender = ChoisirGender(youth.GenderPreference);
        var listePrenoms = gender == "M" ? _spec.Noms.PrenomsMasculins : _spec.Noms.PrenomsFeminins;
        var prenom = ChoisirNom(listePrenoms, $"Prenom{index}");
        var nom = ChoisirNom(_spec.Noms.Noms, $"Nom{index}");

        var specialite = youth.SpecializationPreference != "NONE" && estTrainee
            ? youth.SpecializationPreference.ToLowerInvariant()
            : ChoisirSpecialite(profil.Specialites);
        ...
        var (inRing, entertainment, story) = specialite switch
        {
            ...
            "luchador" => (baseValue + 5, baseValue + 2, baseValue + 1), // Bonus InRing pour Luchador
            "brawler" => (baseValue + 3, baseValue + 1, baseValue + 3), // Mix InRing/Story
            "technician" => (baseValue + 6, baseValue + 0, baseValue + 2),
            _ => (baseValue + 2, baseValue + 2, baseValue + 2)
        };

        AppliquerBonusRegion(youth.Region ?? "Global", ref inRing, ref entertainment, ref story);
        ...
        var workerId = estTrainee
            ...
            : $"FA-{youth.Region ?? "Global"}-{semaine}-{index + 1}";
    ...
    private string ChoisirGender(string preference)
    {
        return preference.ToUpperInvariant() switch
        {
            "MALE" => "M",
            "FEMALE" => "F",
            _ => _random.NextDouble() > 0.5 ? "M" : "F"
        };
    }
