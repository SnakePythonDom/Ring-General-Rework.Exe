...
public sealed class DbInitializer
{
    ...
    public void ApplyMigrations(string cheminDb)
    {
        ...
        StandardizeLegacySchema(connexion);
    ...
    private static void StandardizeLegacySchema(SqliteConnection connexion)
    {
        try
        {
            // 1. Renommage des tables (snake_case -> PascalCase)
            var tableMigrations = new Dictionary<string, string>
            {
                { "youth_structures", "YouthStructures" },
                { "youth_trainees", "YouthTrainees" },
                { "youth_staff_assignments", "YouthStaffAssignments" },
                { "youth_programs", "YouthPrograms" },
                { "youth_generation_state", "YouthGenerationState" }
            };

            foreach (var mig in tableMigrations)
            {
                if (TableExists(connexion, mig.Key))
                {
                    if (!TableExists(connexion, mig.Value))
                    {
                        using var cmd = connexion.CreateCommand();
                        cmd.CommandText = $"ALTER TABLE {mig.Key} RENAME TO {mig.Value};";
                        cmd.ExecuteNonQuery();
                        Console.WriteLine($"Renamed table {mig.Key} to {mig.Value}");
                    }
                    else
                    {
                        // Si les deux existent, on essaie de migrer les données si la nouvelle est vide
                        using var checkCmd = connexion.CreateCommand();
                        checkCmd.CommandText = $"SELECT COUNT(*) FROM {mig.Value}";
                        var count = Convert.ToInt32(checkCmd.ExecuteScalar());

                        if (count == 0)
                        {
                            using var migrateCmd = connexion.CreateCommand();

                            // Mapping explicite car les schémas peuvent différer (vu dans les logs Error 1: column count mismatch)
                            string sql = mig.Value switch
                            {
                                "YouthStructures" =>
                                    "INSERT INTO YouthStructures (YouthStructureId, CompanyId, Name, RegionId, Type, BudgetAnnuel, CapaciteMax, NiveauEquipements, QualiteCoaching, Philosophie, Actif) " +
                                    "SELECT youth_id, company_id, nom, region, type, budget_annuel, capacite_max, niveau_equipements, qualite_coaching, philosophie, actif FROM youth_structures",

                                "YouthTrainees" =>
                                    "INSERT INTO YouthTrainees (YouthTraineeId, YouthStructureId, WorkerId, Status, StartDate, EndDate) " +
                                    "SELECT (worker_id || '_' || youth_id), youth_id, worker_id, statut, semaine_inscription, semaine_graduation FROM youth_trainees",

                                "YouthStaffAssignments" =>
                                    "INSERT INTO YouthStaffAssignments (YouthStaffAssignmentId, YouthStructureId, WorkerId, Role, StartDate) " +
                                    "SELECT assignment_id, youth_id, worker_id, role, semaine_debut FROM youth_staff_assignments",

                                "YouthPrograms" =>
                                    "INSERT INTO YouthPrograms (ProgramId, YouthStructureId, Name, DurationWeeks, FocusAttributes) " +
                                    "SELECT program_id, youth_id, nom, duree_semaines, focus FROM youth_programs",

                                _ => $"INSERT INTO {mig.Value} SELECT * FROM {mig.Key}"
                            };
                            ...
                            try
                            {
                                ...
                                Console.WriteLine($"Migrated data from {mig.Key} to {mig.Value}");
                            }
                            catch (Exception ex)
                            {
                                Console.WriteLine($"Warning: Failed to migrate data for {mig.Key}: {ex.Message}");
                            }
                        }

                        // Toujours supprimer la table legacy si elle contient des données ou non, pour nettoyer
                        try
                        {
                            using var dropCmd = connexion.CreateCommand();
                            dropCmd.CommandText = $"DROP TABLE {mig.Key}";
                            dropCmd.ExecuteNonQuery();
                            Console.WriteLine($"Dropped legacy table {mig.Key}");
                        }
                        catch { /* Ignorer si erreur lors du DROP */ }
                    }
                }
            }

            // 2. Renommage des colonnes
            var tableRenames = new Dictionary<string, Dictionary<string, string>>
            {
                // ... (existing renames)
                ["Shows"] = new()
                {
                ...
                ["YouthStructures"] = new()
                {
                    ...
                    { "annual_budget", "BudgetAnnuel" },
                    { "niveau_equipements", "NiveauEquipements" },
                    { "infrastructure_level", "NiveauEquipements" },
                    { "qualite_coaching", "QualiteCoaching" },
                    { "coaching_quality", "QualiteCoaching" },
                ...
                {
                    { "statut", "Status" },
                    { "status", "Status" }, // Just in case casing
                    { "youth_structure_id", "YouthStructureId" },
                    { "worker_id", "WorkerId" },
                    { "WeekStart", "StartDate" },
                    { "week_start", "StartDate" },
                    { "WeekGraduation", "EndDate" },
                    { "week_graduation", "EndDate" }
                },
                ["YouthStaffAssignments"] = new()
                {
                    { "StartWeek", "StartDate" },
                    { "week_start", "StartDate" },
                    { "youth_structure_id", "YouthStructureId" },
                ...
                ["SegmentTemplates"] = new()
                {
                    ...
                    { "IsMainEvent", "EstMainEvent" },
                    { "Intensity", "Intensite" }
                }
            };

            foreach (var table in tableRenames)
            {
                foreach (var rename in table.Value)
                {
                    if (ColumnExists(connexion, table.Key, rename.Key) && !ColumnExists(connexion, table.Key, rename.Value))
                    {
                        using var cmd = connexion.CreateCommand();
                        cmd.CommandText = $"ALTER TABLE {table.Key} RENAME COLUMN {rename.Key} TO {rename.Value}";
                        cmd.ExecuteNonQuery();
                        Console.WriteLine($"Standardized {table.Key}.{rename.Key} -> {rename.Value}");
                    }
                }
            }

            // 3. Ajout des colonnes manquantes (auto-upgrade)
            var tableAdditions = new Dictionary<string, string[]>
            {
                ["MatchTypes"] = new[]
                {
            ...
            foreach (var table in tableAdditions)
            {
                if (!TableExists(connexion, table.Key)) continue;

                foreach (var colDef in table.Value)
                {
                    var colName = colDef.Split(' ')[0];
                    if (!ColumnExists(connexion, table.Key, colName))
                    {
                        using var cmd = connexion.CreateCommand();
                        cmd.CommandText = $"ALTER TABLE {table.Key} ADD COLUMN {colDef}";
                        cmd.ExecuteNonQuery();
                        Console.WriteLine($"Added missing column {table.Key}.{colName}");
                    }
                }
            }
        }
        ...
        {
            Console.WriteLine($"Warning: Error during schema standardization: {ex.Message}");
        }
    }
