...
public sealed class GameRepository : RepositoryBase, IGameRepository
{
    ...
    public GameRepository(
        ...
        IOwnerDecisionEngine ownerDecisionEngine,
        IBookerAIEngine bookerAIEngine) : base(factory)
    {
    ...
    public IRelationsRepository RelationsRepository => _relationsRepository;
    public IPersonalityEngine PersonalityEngine => _personalityEngine;
    public IOwnerDecisionEngine OwnerDecisionEngine => _ownerDecisionEngine;
    public IBookerAIEngine BookerAIEngine => _bookerAIEngine;
    ...
    public void AddOrUpdateRelation(WorkerRelation relation)
        => _relationsRepository.AddOrUpdateRelation(relation);

    public void TerminateCurrentContract(string workerId, DateTime date)
    {
        _workerRepository.TerminateCurrentContract(workerId, date);
    }

    public IReadOnlyList<BookerMemoryEntry> GetBookerMemory(string bookerId, string? workerId = null)
    {
        return _workerRepository.GetBookerMemory(bookerId, workerId);
    }

    public IReadOnlyList<WorkerRelation> GetWorkerRelations(string workerId)
    {
        return _relationsRepository.GetRelationsForWorker(workerId);
    }
    ...
    public void EnregistrerMatchTypes(IReadOnlyList<MatchTypeDefinition> types)
    {
        ...
        foreach (var type in types)
        {
            ...
            command.CommandText = """
                ...
                """;
            command.Parameters.AddWithValue("$id", type.MatchTypeId ?? string.Empty);
            command.Parameters.AddWithValue("$libelle", type.Libelle ?? string.Empty);
    ...
    public void EnregistrerSegmentTemplates(IReadOnlyList<SegmentTemplateDefinition> templates)
    {
        ...
        {
            var segments = template.Segments ?? Array.Empty<SegmentTemplateSegmentDefinition>();
            var first = segments.FirstOrDefault();

            using var command = connexion.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = """
                INSERT INTO SegmentTemplates (
                    TemplateId, Libelle, Description, SegmentsJson,
                    TypeSegment, DureeMinutes, EstMainEvent, Intensite, MatchTypeId
                )
                VALUES (
                    $id, $libelle, $description, $segments,
                    $type, $duree, $mainEvent, $intensite, $matchTypeId
                )
                ...
                DO UPDATE SET Libelle = $libelle,
                              ...
                              TypeSegment = $type,
                              DureeMinutes = $duree,
                              EstMainEvent = $mainEvent,
                              Intensite = $intensite,
                              MatchTypeId = $matchTypeId;
                """;
            command.Parameters.AddWithValue("$id", template.TemplateId ?? string.Empty);
            command.Parameters.AddWithValue("$libelle", template.Libelle ?? string.Empty);
            command.Parameters.AddWithValue("$description", (object?)template.Description ?? DBNull.Value);
            command.Parameters.AddWithValue("$segments", JsonSerializer.Serialize(segments, _jsonOptions));

            // Legacy columns population (for backward compatibility and NOT NULL constraints)
            command.Parameters.AddWithValue("$type", first?.TypeSegment ?? "match");
            command.Parameters.AddWithValue("$duree", first?.DureeMinutes ?? 10);
            command.Parameters.AddWithValue("$mainEvent", (first?.EstMainEvent ?? false) ? 1 : 0);
            command.Parameters.AddWithValue("$intensite", 50); // Default intensity
            command.Parameters.AddWithValue("$matchTypeId", (object?)first?.MatchTypeId ?? DBNull.Value);
    ...
    public void MettreAJourStatsQuotidiennes(string companyId)
        => _workerRepository.MettreAJourStatsQuotidiennes(companyId);
