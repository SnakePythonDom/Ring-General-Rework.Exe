...
public sealed class TvDealNegotiationViewModel : ViewModelBase
{
    ...
    private int _patience = 100;
    private bool _canNegotiate = true;
    ...
    public bool CanProceedToNextStep => CurrentStep switch
    {
        ...
        2 => CurrentOffer != null,
        3 => CurrentOffer != null && _canNegotiate,
        _ => false
    };
    ...
    {
        get => _patience;
        set => this.RaiseAndSetIfChanged(ref _patience, value);
    }

    public int SlotIndex
    {
        get => (int)Terms.Slot;
        set => Terms = Terms with { Slot = (BroadcastSlot)value };
    }
    ...
    private void Negotiate(decimal increasePercent)
    {
        ...
        try
        {
            ...
            Patience = result.RemainingPatience;

            if (result.IsWalkAway)
            {
                _canNegotiate = false;
                this.RaisePropertyChanged(nameof(CanProceedToNextStep));
                return;
            }
...
/// <summary>
/// Helper simple pour le logging interne au ViewModel
/// </summary>
