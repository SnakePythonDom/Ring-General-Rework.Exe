...
public sealed class WorldDbInitializer
{
    ...
    private void ImportAndAdaptDatabase()
    {
        ...
        try
        {
            // 1. Recréer le schéma (DROP et CREATE)
            DropAndCreateWorldSchema(worldConnection);
    ...
    /// <summary>
    /// DROP les tables existantes et recréé le schéma complet
    /// </summary>
    private static void DropAndCreateWorldSchema(SqliteConnection connection)
    {
        try
        {
            ...
            // DROP les tables si elles existent
            var dropScript = """
                DROP TABLE IF EXISTS CompanyStyleEvolution;
                DROP TABLE IF EXISTS CatchStyles;
        ...
        {
            throw new InvalidOperationException($"Erreur lors du DROP/CREATE du schéma: {ex.Message}", ex);
        }
    }
    ...
    private static void ImportRegions(SqliteConnection source, SqliteConnection dest)
    {
        try
        {
            ...
            cmd.CommandText = """
                INSERT OR IGNORE INTO Regions (RegionId, Name, CountryId, CreatedAt) VALUES
                ('USA', 'United States', 'USA', datetime('now')),
                ('JAPAN', 'Japan', 'JAPAN', datetime('now')),
                ('MEXICO', 'Mexico', 'MEXICO', datetime('now')),
                ('CANADA', 'Canada', 'CANADA', datetime('now')),
                ('EUROPE', 'Europe', 'EUROPE', datetime('now'));
                """;
            cmd.ExecuteNonQuery();
        }
        catch
    ...
    private static void ImportCatchStyles(SqliteConnection dest)
    {
        try
        {
            ...
            cmd.CommandText = """
                ...
                ('STYLE_STRONG_STYLE', 'Strong Style', 
                 ...
                 85, 30, 30, 5, 95, 85, 40, 25, 50, 1.3, 0.7, '??', '#9C27B0', 1, datetime('now')),
        ...
        catch (Exception ex)
        {
            ...
            System.Diagnostics.Debug.WriteLine($"?? Erreur lors de l'import des styles de catch: {ex.Message}");
        }
    }
