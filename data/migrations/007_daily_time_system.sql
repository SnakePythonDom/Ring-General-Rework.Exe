...
SET MonthlyWage = CASE 
    WHEN Salary > 0 THEN Salary * 4.0  -- Convertir salaire hebdomadaire en mensuel
    ELSE 0
END,
AppearanceFee = CASE
    WHEN BonusShow > 0 THEN BonusShow  -- BonusShow devient AppearanceFee
    ELSE 0
END
WHERE MonthlyWage = 0;
...
CREATE INDEX IF NOT EXISTS idx_contracts_monthly_wage 
    ON contracts(CompanyId, MonthlyWage) WHERE MonthlyWage > 0;
CREATE INDEX IF NOT EXISTS idx_contracts_appearance_fee 
    ON contracts(CompanyId, AppearanceFee) WHERE AppearanceFee > 0;
CREATE INDEX IF NOT EXISTS idx_contracts_last_payment 
    ON contracts(CompanyId, LastPaymentDate);
CREATE INDEX IF NOT EXISTS idx_contracts_last_appearance 
    ON contracts(CompanyId, LastAppearanceDate);
