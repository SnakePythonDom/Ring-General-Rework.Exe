    ...
    public IReadOnlyList<TvDeal> ChargerTvDeals(string companyId)
    {
        ...
        command.CommandText = """
            SELECT tv_deal_id, company_id, network_name, reach_bonus, audience_cap, audience_min,
                   base_revenue, revenue_per_point, penalty, constraints, SlotType, Distribution, ProductionRequirement
            FROM tv_deals
            WHERE company_id = $companyId
        ...
        while (reader.Read())
        {
            deals.Add(new TvDeal
            {
                ...
                Slot = Enum.TryParse<BroadcastSlot>(reader.GetString(10), out var s) ? s : BroadcastSlot.Daytime,
                Distribution = Enum.TryParse<DistributionType>(reader.GetString(11), out var d) ? d : DistributionType.Local,
                ProductionRequirement = reader.GetInt32(12)
            });
        }
        ...
        try
        {
            ...
            command.CommandText = """
                INSERT INTO tv_deals (
                    ...
                    SlotType, Distribution, ProductionRequirement
                ) VALUES (
            ...
            command.Parameters.AddWithValue("$slot", deal.Slot.ToString());
            command.Parameters.AddWithValue("$distribution", deal.Distribution.ToString());
            command.Parameters.AddWithValue("$production", deal.ProductionRequirement);
            command.ExecuteNonQuery();

            // Régions
            foreach (var regionId in deal.TargetRegionIds)
            {
                using var regionCmd = connexion.CreateCommand();
                regionCmd.Transaction = dbTransaction;
                regionCmd.CommandText = "INSERT INTO TvDealRegions (TvDealId, RegionId) VALUES ($dealId, $regionId)";
                regionCmd.Parameters.AddWithValue("$dealId", deal.TvDealId);
                regionCmd.Parameters.AddWithValue("$regionId", regionId);
    ...
    private static IReadOnlyList<ContractPayroll> ChargerPaieContratsUpper(SqliteConnection connexion, string companyId)
    {
        ...
        command.CommandText = $"""
            SELECT contracts.worker_id,
                   COALESCE(workers.LastName || ' ' || workers.FirstName, contracts.worker_id),
                   {salaryColumn},
                   'Hebdomadaire'
            FROM contracts
