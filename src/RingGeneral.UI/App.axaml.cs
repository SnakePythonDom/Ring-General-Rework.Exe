...
public sealed class App : Avalonia.Application
{
    ...
    public override void OnFrameworkInitializationCompleted()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                // Ensure migrations are applied even if DB exists (auto-update schema)
                try
                {
                    logger.Info("Vérification des migrations...");
                    var tempInitializer = new DbInitializer();
                    tempInitializer.ApplyMigrations(generalDbPath);
                    logger.Info("Migrations vérifiées/appliquées.");
                }
                catch (Exception ex)
                {
                    logger.Error($"Erreur lors de l'application des migrations: {ex.Message}");
                }
            }
        ...
        services.AddSingleton<IOwnerDecisionEngine>(sp =>
            new OwnerDecisionEngine(sp.GetRequiredService<CoreInterfaces.IOwnerRepository>()));

        services.AddSingleton<IBookerAIEngine>(sp =>
            new BookerAIEngine(
                sp.GetRequiredService<CoreInterfaces.IBookerRepository>(),
                sp.GetRequiredService<IRelationsRepository>()));

        // Phase 3: The Network (Recruitment & Staff Sharing)
        services.AddSingleton<IRecruitmentScoringService>(sp =>
            new RecruitmentScoringService(
                sp.GetRequiredService<IGameRepository>(),
                sp.GetRequiredService<ICompanyRepository>(),
                sp.GetRequiredService<ICatchStyleRepository>()));

        services.AddSingleton<IStaffSharingService>(sp =>
            new StaffSharingService(
                sp.GetRequiredService<IGameRepository>(),
                sp.GetRequiredService<IChildCompanyStaffRepository>()));
        ...
        services.AddSingleton<IWorkerRepository>(repositories.WorkerRepository);
        ...
        // Staff Storyline Integration
        services.AddSingleton<StaffStorylineIntegration>();
        ...
        // Worker Generation Service (Phase 4.2)
        services.AddSingleton<WorkerGenerationSpec>(sp =>
        {
            var paths = new[]
            {
            ...
            var path = paths.FirstOrDefault(File.Exists) ?? throw new FileNotFoundException("Worker generation spec not found.");
            var json = File.ReadAllText(path);
            return JsonSerializer.Deserialize<WorkerGenerationSpec>(json, new JsonSerializerOptions { PropertyNameCaseInsensitive = true })!;
        });
        services.AddSingleton<IRandomProvider>(sp => new SeededRandomProvider(42));
        services.AddSingleton<IWorkerGenerationService, WorkerGenerationService>();
        ...
        services.AddTransient<YouthViewModel>(sp =>
            new YouthViewModel(
                ...
                sp.GetRequiredService<IWorkerGenerationService>()));
        services.AddTransient<YouthHubViewModel>(sp =>
            new YouthHubViewModel(
                sp.GetRequiredService<GameRepository>(),
                sp.GetRequiredService<YouthRepository>(),
                sp.GetRequiredService<IRegionRepository>(),
                sp.GetRequiredService<IWorkerGenerationService>()));
        ...
        services.AddTransient<CalendarViewModel>(sp =>
            new CalendarViewModel(
                ...
                sp.GetRequiredService<RingGeneral.Core.Services.ShowSchedulerService>(),
                sp.GetRequiredService<ShowRepository>()));
        services.AddTransient<FreeAgentsViewModel>(sp =>
            new FreeAgentsViewModel(
                "PLAYER_COMPANY_ID", // TODO: Get from session
                sp.GetRequiredService<IFreeAgentRepository>(),
                sp.GetRequiredService<IGameRepository>(),
                sp.GetRequiredService<IRecruitmentScoringService>(),
                sp.GetRequiredService<IEventAggregator>()));
        ...
        services.AddTransient<ViewModels.Contracts.ContractNegotiationViewModel>();
    ...
    private static SegmentTypeCatalog ChargerSegmentTypes()
    {
        ...
        return new SegmentTypeCatalog(
            ...
            {
                ["match"] = new List<string> { "style", "outcome" },
                ["promo"] = new List<string> { "focus" }
            },
            new Dictionary<string, IReadOnlyList<string>>
            {
                ["style"] = new List<string> { "Standard", "Technical", "Brawl", "Hardcore", "High Flying", "Squash" },
                ["outcome"] = new List<string> { "Pinfall", "Submission", "DQ", "Count Out", "Draw", "No Contest" },
                ["focus"] = new List<string> { "Self", "Opponent", "Storyline", "Crowd" }
            },
            new Dictionary<string, string>
            {
                ["style"] = "Style de match",
                ["outcome"] = "Type de fin",
