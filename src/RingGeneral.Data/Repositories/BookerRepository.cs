...
public sealed class BookerRepository : RepositoryBase, RingGeneral.Core.Interfaces.IBookerRepository
{
    ...
    public async Task SaveBookerMemoryAsync(BookerMemory memory)
    {
        ...
        command.CommandText = @"
            INSERT INTO BookerMemory (
                MemoryId, BookerId, CompanyId, WorkerId, EventType, EventDescription, ImpactScore, RecallStrength, CreatedAt, EventDate
            ) VALUES (
                @MemoryId, @BookerId, @CompanyId, @WorkerId, @EventType, @EventDescription, @ImpactScore, @RecallStrength, @CreatedAt, @EventDate
            )";

        command.Parameters.AddWithValue("@MemoryId", string.IsNullOrEmpty(memory.MemoryId) ? Guid.NewGuid().ToString("N") : memory.MemoryId);
        command.Parameters.AddWithValue("@BookerId", memory.BookerId);
        command.Parameters.AddWithValue("@CompanyId", memory.CompanyId);
        command.Parameters.AddWithValue("@WorkerId", (object?)memory.WorkerId ?? DBNull.Value);
        ...
        command.Parameters.AddWithValue("@EventDate", memory.CreatedAt.ToString("O")); // Using CreatedAt as EventDate if not split
    ...
    public async Task<List<BookerMemory>> GetBookerMemoriesAsync(string bookerId)
    {
        ...
        using var command = connection.CreateCommand();
        command.CommandText = @"
            SELECT MemoryId, BookerId, CompanyId, WorkerId, EventType, EventDescription, ImpactScore, RecallStrength, CreatedAt
            FROM BookerMemory
            WHERE BookerId = @BookerId
    ...
    async Task<BookerMemory?> Core.Interfaces.IBookerRepository.GetBookerMemoryByIdAsync(string memoryId)
    {
        using var connection = new SqliteConnection(_connectionString);
        await connection.OpenAsync();
    ...
    private static BookerMemory MapBookerMemory(SqliteDataReader reader)
    {
        return new BookerMemory
        {
            ...
            WorkerId = reader.IsDBNull(3) ? null : reader.GetString(3),
            EventType = reader.GetString(4),
            EventDescription = reader.GetString(5),
            ImpactScore = reader.GetInt32(6),
            RecallStrength = reader.GetInt32(7),
