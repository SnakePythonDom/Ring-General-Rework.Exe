...
public static class DbSeeder
{
    ...
    public static void SeedIfEmpty(SqliteConnection connection)
    {
        ...
        {
            Log(LogLevel.Info, "Base de données n'est pas vide, seeding principal ignoré");
            // Mais vérifier quand même si on doit seeder la partie Youth si elle est vide
            SeedYouthDataIfNeeded(connection);
            return;
        }
    ...
    /// <summary>
    /// Seed spécifiquement les données Youth et Filiales si absentes
    /// </summary>
    private static void SeedYouthDataIfNeeded(SqliteConnection connection)
    {
        try
        {
            using var cmd = connection.CreateCommand();
            cmd.CommandText = "SELECT COUNT(*) FROM YouthStructures";
            var structureCount = Convert.ToInt64(cmd.ExecuteScalar());

            if (structureCount == 0)
            {
                Log(LogLevel.Info, "Aucune structure Youth détectée, seeding des données Youth...");

                // Récupérer la première compagnie existante pour lui lier les données
                using var compCmd = connection.CreateCommand();
                compCmd.CommandText = "SELECT CompanyId FROM Companies LIMIT 1";
                var companyId = compCmd.ExecuteScalar()?.ToString();

                if (string.IsNullOrEmpty(companyId))
                {
                    Log(LogLevel.Warning, "Impossible de seeder Youth : Aucune compagnie trouvée.");
                    return;
                }

                // Récupérer quelques workers pour les trainees/staff
                var workerIds = new List<string>();
                using var workerCmd = connection.CreateCommand();
                workerCmd.CommandText = "SELECT WorkerId FROM Workers LIMIT 20";
                ...
                {
                    SeedYouthData(connection, companyId, workerIds);
                    Log(LogLevel.Info, "✅ Seeding Youth terminé.");
                }
            }
        }
        catch (Exception ex)
        {
            Log(LogLevel.Warning, $"Erreur lors de la vérification/seeding Youth : {ex.Message}");
        }
    }
    ...
    private static void SeedDemoData(SqliteConnection connection)
    {
        ...
        try
        {
            ...
            // 5. Créer des filiales et du contenu Youth (Centre de Développement)
            try
            {
                SeedYouthData(connection, companyId, workerIds);
            }
            ...
            {
                Log(LogLevel.Warning, $"Erreur lors du seeding Youth : {ex.Message}");
            }
    ...
    /// <summary>
    /// Crée des données pour le Centre de Développement (Filiales, Structures, Trainees)
    /// </summary>
    private static void SeedYouthData(SqliteConnection connection, string companyId, List<string> workerIds)
    {
        Log(LogLevel.Info, "Démarrage du seeding Youth...");

        // 1. Créer une filiale de développement
        var childCompanyId = "CHILD_NXT";
        if (TableExists(connection, "ChildCompanies"))
        {
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = @"
                    INSERT OR IGNORE INTO ChildCompanies (ChildCompanyId, ParentCompanyId, Name, RegionId, Level, MonthlyBudget)
                    VALUES (@id, @parentId, @name, @regionId, @level, @budget)";
                cmd.Parameters.AddWithValue("@id", childCompanyId);
                cmd.Parameters.AddWithValue("@parentId", companyId);
                cmd.Parameters.AddWithValue("@name", "NXT Wrestling");
                cmd.Parameters.AddWithValue("@regionId", "REGION_USA_DEFAULT");
                cmd.Parameters.AddWithValue("@level", "Development");
                cmd.Parameters.AddWithValue("@budget", 50000.0);
                cmd.ExecuteNonQuery();
            }
            Log(LogLevel.Info, $"Filiale créée : {childCompanyId}");
        }

        // 2. Créer une structure Youth (Dojo)
        var youthId = "YOUTH_PC";
        if (TableExists(connection, "YouthStructures"))
        {
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = @"
                    INSERT OR IGNORE INTO YouthStructures (YouthId, CompanyId, Nom, Type, RegionId, BudgetAnnuel, CapaciteMax, NiveauEquipements, QualiteCoaching, Philosophie, Actif)
                    VALUES (@id, @companyId, @nom, @type, @regionId, @budget, @capacity, @equip, @coach, @philo, @actif)";
                cmd.Parameters.AddWithValue("@id", youthId);
                cmd.Parameters.AddWithValue("@companyId", companyId);
                cmd.Parameters.AddWithValue("@nom", "WWE Performance Center");
                cmd.Parameters.AddWithValue("@type", "DOJO");
                cmd.Parameters.AddWithValue("@regionId", "REGION_USA_DEFAULT");
                cmd.Parameters.AddWithValue("@budget", 100000);
                cmd.Parameters.AddWithValue("@capacity", 20);
                cmd.Parameters.AddWithValue("@equip", 85);
                cmd.Parameters.AddWithValue("@coach", 90);
                cmd.Parameters.AddWithValue("@philo", "PURE");
                cmd.Parameters.AddWithValue("@actif", 1);
                cmd.ExecuteNonQuery();
            }
            Log(LogLevel.Info, $"Structure Youth créée : {youthId}");
        }

        // 3. Ajouter des trainees (les 5 derniers workers par exemple)
        if (TableExists(connection, "YouthTrainees") && workerIds.Count >= 5)
        {
            var trainees = workerIds.Skip(workerIds.Count - 5).ToList();
            foreach (var workerId in trainees)
            {
                using (var cmd = connection.CreateCommand())
                {
                    cmd.CommandText = @"
                        INSERT OR IGNORE INTO YouthTrainees (WorkerId, YouthStructureId, Status, StartDate)
                        VALUES (@workerId, @youthId, 'EN_FORMATION', @start)";
                    cmd.Parameters.AddWithValue("@workerId", workerId);
                    cmd.Parameters.AddWithValue("@youthId", youthId);
                    cmd.Parameters.AddWithValue("@start", 1); // Semaine 1
                    cmd.ExecuteNonQuery();
                }
            }
            Log(LogLevel.Info, $"{trainees.Count} trainees ajoutés à {youthId}");
        }

        // 4. Ajouter du staff
        if (TableExists(connection, "YouthStaffAssignments") && workerIds.Count >= 2)
        {
            var staffWorkerId = workerIds[8]; // HBK example
            using (var cmd = connection.CreateCommand())
            {
                cmd.CommandText = @"
                    INSERT OR IGNORE INTO YouthStaffAssignments (YouthId, WorkerId, Role, StartDate)
                    VALUES (@youthId, @workerId, @role, @start)";
                cmd.Parameters.AddWithValue("@youthId", youthId);
                cmd.Parameters.AddWithValue("@workerId", staffWorkerId);
                cmd.Parameters.AddWithValue("@role", "ENTRAINEUR_CHEF");
                cmd.Parameters.AddWithValue("@start", 1);
                cmd.ExecuteNonQuery();
            }
            Log(LogLevel.Info, $"Staff ajouté à {youthId} : {staffWorkerId}");
        }
    }
