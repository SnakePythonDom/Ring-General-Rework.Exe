...
public sealed class BookingControlService : IBookingControlService
{
    ...
    public bool IsActionAllowed(BookingControlLevel level, BookingAction action)
    {
        return level switch
        {
            BookingControlLevel.Spectator => action == BookingAction.SimulateShow,
            BookingControlLevel.Producer => action != BookingAction.MoveSegment, // Producer can edit what IA proposed but not restructure radically?
            BookingControlLevel.CoBooker => true,
            BookingControlLevel.Dictator => action != BookingAction.VetoProposal,
            _ => true
        };
    }
}
