...
public sealed class OwnerDecisionEngine : IOwnerDecisionEngine
{
    ...
    public bool ApprovesBudget(string ownerId, decimal proposedBudget, decimal companyTreasury)
    {
        ...
        // Limite de budget basée sur RiskTolerance et VisionType
        // Un owner Creative acceptera plus de budget s'il s'agit de production
        // Un owner Business sera plus strict sur le ratio trésorerie
        var maxBudgetPercentage = owner.RiskTolerance / 2.0 + (owner.VisionType == "Creative" ? 10 : 0);

        // Si le budget proposé dépasse 10% de la trésorerie et que le risque est > RiskTolerance
        if (budgetPercentage > (decimal)maxBudgetPercentage)
        {
            return false;
        }
        ...
        int rosterMorale)
    {
        ...
        // Si satisfaction fans critique (< 40), priorité fan satisfaction
        if (fanSatisfaction < 40)
        {
    ...
    public bool ShouldReplaceBooker(string ownerId, int bookerPerformance, int monthsEmployed)
    {
        ...
        // CRITÈRE NÉPOTISME: Si l'impact cumulé du népotisme est trop élevé (> 80)
        // Note: Dans une version future, on injectera le NepotismEngine ici
        // if (totalNepotismImpact > 80) return true;
    ...
    public void SetGoal(string ownerId, OwnerGoal goal)
    {
        if (_ownerRepository == null) return;
        goal.OwnerId = ownerId;
        _ownerRepository.AddGoalAsync(goal).Wait();
    }

    public IEnumerable<OwnerGoal> GetGoals(string ownerId)
    {
        if (_ownerRepository == null) return Enumerable.Empty<OwnerGoal>();
        return _ownerRepository.GetGoalsAsync(ownerId).Result;
    }

    public void EvaluateGoals(string ownerId, DateTime currentDate)
    {
        if (_ownerRepository == null) return;
        var goals = GetGoals(ownerId).ToList();
        var owner = _ownerRepository.GetOwnerByIdAsync(ownerId).Result;
        if (owner == null) return;

        int satisfactionLoss = 0;
        ...
        foreach (var goal in goals)
        {
            ...
            if (goal.CurrentValue >= goal.TargetValue)
            {
                ...
                _ownerRepository.UpdateGoalAsync(goal).Wait();
            }
            else if (currentDate > goal.Deadline)
            {
                goal.Status = GoalStatus.Failed;
                _ownerRepository.UpdateGoalAsync(goal).Wait();
                // Consequence: perte de satisfaction basée sur l'importance de l'objectif
                satisfactionLoss += Math.Max(5, goal.Importance / 10);
            }
        }

        if (satisfactionLoss > 0)
        {
            var newSatisfaction = Math.Max(0, owner.Satisfaction - satisfactionLoss);
            var updatedOwner = owner with { Satisfaction = newSatisfaction };
            _ownerRepository.UpdateOwnerAsync(updatedOwner).Wait();

            System.Diagnostics.Debug.WriteLine($"Owner {ownerId} satisfaction decreased by {satisfactionLoss} due to failed goals. New satisfaction: {newSatisfaction}");
        }
    }
}
