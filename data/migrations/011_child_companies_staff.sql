...
ALTER TABLE Workers ADD COLUMN CanBeShared INTEGER DEFAULT 1;
ALTER TABLE Workers ADD COLUMN MobilityRating TEXT DEFAULT 'MEDIUM';
ALTER TABLE Workers ADD COLUMN SharingPreferences TEXT; -- JSON
ALTER TABLE Workers ADD COLUMN ChildSpecializations TEXT; -- JSON
...
CREATE INDEX IF NOT EXISTS idx_child_staff_assignments_staff ON ChildCompanyStaffAssignments(WorkerId);
...
-- Indexes Planning
CREATE INDEX IF NOT EXISTS idx_staff_schedules_staff ON StaffSharingSchedules(WorkerId, WeekNumber);

-- Indexes Impacts
CREATE INDEX IF NOT EXISTS idx_staff_impacts_staff ON StaffProgressionImpacts(WorkerId);
