...
public class PersonalityRepository : RepositoryBase, IPersonalityRepository
{
    ...
    public async Task InitializePersonalitySystemAsync(string entityId, string entityType)
    {
        ...
        // Générer attributs mentaux (Format Engine / WorkerMentalAttributes)
        var engineAttributes = _personalityEngine.GenerateRandomMentalAttributes();

        // Calculer label de personnalité (requiert WorkerMentalAttributes)
        var personalityLabel = _personalityEngine.CalculatePersonalityLabel(engineAttributes);

        // Générer traits secondaires (requiert WorkerMentalAttributes)
        var secondaryTraits = _personalityEngine.GenerateSecondaryTraits(engineAttributes);

        // Mapper vers le format de stockage (MentalAttributes)
        var storageAttributes = new MentalAttributes
        {
            EntityId = entityId,
            EntityType = entityType,
            Professionalism = engineAttributes.Professionnalisme,
            Ambition = engineAttributes.Ambition,
            Loyalty = engineAttributes.Loyauté,
            Ego = engineAttributes.Égoïsme,
            Resilience = engineAttributes.Détermination,
            Adaptability = engineAttributes.Pression, // Mapping Pression -> Adaptability
            SocialSkills = engineAttributes.Sportivité, // Mapping Sportivité -> SocialSkills
            Temperament = engineAttributes.Tempérament,

            // Valeurs par défaut pour les champs non présents dans l'ancien modèle
            WorkEthic = engineAttributes.Professionnalisme,
            Creativity = 10,
        ...
        // Sauvegarder
        await SaveMentalAttributesAsync(storageAttributes);
