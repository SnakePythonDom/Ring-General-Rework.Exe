...
public sealed class SaveGameManager
{
    ...
    public SaveGameInfo CreerNouvellePartie(string companyId, string companyName, string? playerId = null, string difficulty = "Normal", string gameMode = "Standard")
    {
        ...
        var dateFormatted = "2024-01-01"; // Date de départ par défaut
        ...
        try
        {
            ...
            using (var insertCmd = conn.CreateCommand())
            {
                ...
                insertCmd.Parameters.AddWithValue("@saveName", $"{companyName} - {now:yyyy-MM-dd HH:mm}");
            ...
            return new SaveGameInfo(
                SaveId: "CURRENT", // On simplifie pour l'instant
                Name: companyName,
                FullPath: _connectionFactory.GeneralDatabasePath,
                LastWriteTime: now,
                CompanyId: companyId,
                WorldVersion: 1,
                IsFromSaveDb: true,
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException($"Erreur lors de la création de la sauvegarde: {ex.Message}", ex);
        }
    }

    /// <summary>
    /// ✅ Charge la sauvegarde active depuis GENERAL DB.
    /// </summary>
    public SaveGameInfo? ChargerSauvegardeActive()
    {
        ...
        var saveId = reader.GetInt32(0).ToString();
    ...
    /// <summary>
    /// ✅ Met à jour une sauvegarde active.
    /// </summary>
    public void MettreAJourSauvegarde(string companyId, int currentDay, string currentDate)
    {
        using var conn = _connectionFactory.CreateGeneralConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
    ...
    /// <summary>
    /// ✅ Liste toutes les sauvegardes du GENERAL DB.
    /// </summary>
    public IReadOnlyList<SaveGameInfo> ListerSauvegardesActives()
    {
        ...
        while (reader.Read())
        {
            saves.Add(new SaveGameInfo(
                SaveId: reader.GetInt32(0).ToString(),
